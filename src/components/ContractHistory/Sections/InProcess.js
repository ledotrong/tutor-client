...
const count = 3;
export default class InProcess extends React.Component{
    state = {
        page: 0,
        initLoading: true,
        loading: false,
            ...
            renderItem={item =>{
              ...
              >
                <Skeleton avatar title={false} loading={item.loading} active>
                  <List.Item.Meta 
                    title={<Link to={`/userdetail/${role === "student"? item.tutorID:item.studentID}`}><a style={{padding:"0px 30px"}}>{role === "student"? item.tutorName: item.studentName}</a></Link>}
                  />
