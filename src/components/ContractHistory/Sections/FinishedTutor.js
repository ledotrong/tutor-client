...
const count = 3;
export default class FinishedTutor extends React.Component{
    state = {
        page: 0,
        initLoading: true,
        loading: false,
      ...
      !initLoading && !loading ? (
        ...
        return(
            ...
            renderItem={item =>{
              ...
              <List.Item 
              key={item._id}
                actions={[<span style={{padding:"0px 30px", marginBottom:"50px"}}>Student has not confirmed yet.</span>,
         ...
         <List.Item 
         key={item._id}
           actions={[<span style={{padding:"0px 30px", marginBottom:"50px"}}>Student had confirmed.</span>, <a style={{padding:"0px 30px", marginBottom:"50px" }} onClick={()=>this.viewDetail(item)}><IconText  type="info-circle" text="View Detail" key="viewdetail" /></a>]}
         >
           <Skeleton avatar title={false} loading={item.loading} active>
             <List.Item.Meta 
               title={<Link to={`/userdetail/${item.studentID}`}><a style={{padding:"0px 30px"}}>{item.studentName}</a></Link>}
